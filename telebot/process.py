...
from .config import CONFIG_DIR, MAIN_SCRIPT, VENV_PYTHON, SILENT_CONFIG, get_api_key_file
...
def add_bot(bot_id, apikey, secret):
    ...
    bot_config = CONFIG_DIR + f"{bot_id}.json"
    if not os.path.exists(bot_config):
        shutil.copy(SILENT_CONFIG, bot_config)
    upsert_bot(bot_id, bot_config, apikey, secret)